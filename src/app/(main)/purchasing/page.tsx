...
const PRIORITY_CONFIG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  ...
  LOW:    { label: 'Rendah',  color: 'text-muted-foreground',  bg: 'bg-muted',  border: 'border-border' },
};

const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  NEEDED:    { label: 'Dibutuhkan',  color: 'text-amber-700', bg: 'bg-amber-50' },
  ORDERED:   { label: 'Dipesan',     color: 'text-primary',bg: 'bg-muted/50' },
...
export default function PurchasingPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* Header */}
      <div className="pb-2">
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Divisi Purchasing</h1>
        <p className="text-muted-foreground mt-1">Daftar kebutuhan belanja, monitoring stok, dan riwayat pembelian.</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-muted-foreground text-sm font-medium">Item Perlu Dibeli</p>
              <p className="text-4xl font-black text-foreground mt-1">{pendingCount}</p>
            </div>
            <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center"><AlertTriangle className="w-6 h-6" /></div>
          </div>
        </div>
        <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-muted-foreground text-sm font-medium">Total Request Belanja</p>
              <p className="text-4xl font-black text-foreground mt-1">{needs.length}</p>
            </div>
            <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center"><Package className="w-6 h-6" /></div>
          </div>
        </div>
        {isPurchasingOrAbove && (
          <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-black text-foreground mt-1">Rp {totalSpend.toLocaleString('id-ID')}</p>
              </div>
      ...
      {recommendations.length > 0 && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              <p className="font-bold text-amber-800 dark:text-muted-foreground">Rekomendasi Belanja Otomatis</p>
            ...
            {recommendations.map(item => (
              <div key={item.id} className={`flex items-center gap-3 p-3 rounded-xl border ${
                item.priority === 'HIGH'
                  ...
                  : 'bg-white border-amber-100 /50 dark:border-amber-800/30'
              }`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">Sisa: {item.currentStock} {item.unit} · Rekomendasi beli: <span className="font-bold text-amber-700 dark:text-amber-300">{item.recommendedQty} {item.unit}</span></p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex gap-2 bg-muted/50 p-1 rounded-xl w-fit">
        <button onClick={() => setActiveTab('needs')}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'needs' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
          ?? Kebutuhan Belanja ({pendingCount} pending)
        </button>
        {isPurchasingOrAbove && (
          <button onClick={() => setActiveTab('purchases')}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'purchases' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
            ?? Riwayat Pembelian
          </button>
        )}
      </div>
      ...
      {activeTab === 'needs' && (
        <div className="grid gap-6 lg:grid-cols-5">
          <Card className="lg:col-span-2 border-border shadow-md rounded-2xl overflow-hidden">
            <CardHeader className="bg-card/50 border-b border-border p-6">
              <CardTitle className="text-xl font-bold text-foreground flex items-center gap-2">
                <Plus className="w-5 h-5 text-primary" /> Ajukan Kebutuhan
              </CardTitle>
            ...
            <CardContent className="p-6 space-y-5">
              <form onSubmit={handleAddNeed} className="space-y-4">
                <div className="space-y-2">
                  <Label className="font-medium text-foreground">Nama Barang <span className="text-rose-500">*</span></Label>
                  <Input required value={needForm.itemName} onChange={e => setNeedForm({...needForm, itemName: e.target.value})} placeholder="Contoh: Tepung Terigu 25kg" className="rounded-xl border-border bg-muted/30  focus-visible:ring-primary" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={needForm.notes} onChange={e => setNeedForm({...needForm, notes: e.target.value})} placeholder="Merk tertentu, spesifikasi..." className="rounded-xl border-border bg-muted/30  focus-visible:ring-primary" />
                </div>
                <button type="submit" disabled={isSubmitting} className="w-full h-11 inline-flex items-center justify-center gap-2 rounded-xl bg-primary hover:bg-primary/90 text-white font-semibold transition-colors shadow-md disabled:opacity-50">
                  <Plus className="w-4 h-4" />{isSubmitting ? 'Menyimpan...' : 'Tambah Request'}
                </button>
              </form>
            </CardContent>
          </Card>
          ...
          <Card className="lg:col-span-3 border-border shadow-md rounded-2xl overflow-hidden">
            <CardHeader className="bg-card/50 border-b border-border p-6">
              <CardTitle className="text-xl font-bold text-foreground">Daftar Request Belanja</CardTitle>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                {needs.length === 0 ? (
                  <div className="flex flex-col items-center py-16 text-center">
                    ...
                    <p className="text-muted-foreground font-medium">Tidak ada request belanja</p>
                  </div>
                ) : needs.map(need => {
                  const pCfg = PRIORITY_CONFIG[need.priority] || PRIORITY_CONFIG['MEDIUM'];
                  const sCfg = STATUS_CONFIG[need.status] || STATUS_CONFIG['NEEDED'];
                  return (
                    <div key={need.id} className="flex justify-between items-start px-6 py-4 hover:bg-muted/50 transition-colors gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <span className="font-semibold text-foreground">{need.itemName}</span>
                          <span className="text-sm text-muted-foreground">×{need.quantity}</span>
                        ...
                        {need.notes && <p className="text-xs text-muted-foreground">{need.notes}</p>}
                      </div>
                      {isPurchasingOrAbove && need.status !== 'PURCHASED' && (
                        <div className="flex gap-2 shrink-0">
                          {need.status === 'NEEDED' && (
                            <button onClick={() => updateStatus(need.id, 'ORDERED')} className="px-3 py-1.5 text-xs font-semibold rounded-lg border border-primary/30 bg-muted/50 text-primary hover:bg-primary/10 transition-colors">
                              Dipesan
                            </button>
                          )}
        ...
        <div className="grid gap-6 lg:grid-cols-5">
          {isPurchasingOrAbove && (
            <Card className="lg:col-span-2 border-border shadow-md rounded-2xl overflow-hidden">
              <CardHeader className="bg-card/50 border-b border-border p-6">
                <CardTitle className="text-xl font-bold text-foreground flex items-center gap-2">
                  <ShoppingCart className="w-5 h-5 text-primary" /> Catat Pembelian
                </CardTitle>
              </CardHeader>
              <CardContent className="p-6">
                <form onSubmit={handleAddPurchase} className="space-y-4">
                  <div className="space-y-2">
                    <Label className="font-medium text-foreground">Tanggal Beli</Label>
                    <Input type="date" required value={purchaseForm.date} onChange={e => setPurchaseForm({...purchaseForm, date: e.target.value})} className="rounded-xl border-border bg-muted/30  focus-visible:ring-primary" />
                  </div>
                  <div className="space-y-2">
                    <Label className="font-medium text-foreground">Nama Barang <span className="text-rose-500">*</span></Label>
                    <Input required value={purchaseForm.itemName} onChange={e => setPurchaseForm({...purchaseForm, itemName: e.target.value})} className="rounded-xl border-border bg-muted/30  focus-visible:ring-primary" />
                  </div>
                  ...
                  <div className="space-y-2">
                    <Label className="font-medium text-foreground">Supplier / Toko</Label>
                    <Input value={purchaseForm.supplier} onChange={e => setPurchaseForm({...purchaseForm, supplier: e.target.value})} placeholder="Contoh: Toko Makmur" className="rounded-xl border-border bg-muted/30  focus-visible:ring-primary" />
                  </div>
          ...
          <Card className={`${isPurchasingOrAbove ? 'lg:col-span-3' : 'lg:col-span-5'} border-border shadow-md rounded-2xl overflow-hidden`}>
            <CardHeader className="bg-card/50 border-b border-border p-6">
              <CardTitle className="text-xl font-bold text-foreground">Buku Riwayat Belanja</CardTitle>
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-muted/30 border-b border-border">
                  <tr>
                    ...
                    <th className="px-6 py-3 font-semibold text-foreground">Item & Supplier</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {purchases.length === 0 ? (
                    <tr><td colSpan={4} className="px-6 py-12 text-center text-muted-foreground">Belum ada riwayat belanja</td></tr>
                  ) : purchases.map(p => (
                    <tr key={p.id} className="hover:bg-muted/50 transition-colors">
                      <td className="px-6 py-4 text-muted-foreground whitespace-nowrap">{format(new Date(p.date), 'dd MMM yyyy', { locale: localeId })}</td>
                      <td className="px-6 py-4">
                        <p className="font-semibold text-foreground">{p.itemName}</p>
                        <p className="text-xs text-muted-foreground">Toko: {p.supplier || '—'}</p>
                      </td>
                      <td className="px-6 py-4 text-right text-muted-foreground">{p.quantity}</td>
